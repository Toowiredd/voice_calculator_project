...
const service = {
  ...
  userCommands: () => FIND_MANY_RESULT,
  userCommand: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("UserCommand", () => {
  ...
  test("GET /userCommands/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/userCommands"}/${nonExistingId}`)
  ...
  test("GET /userCommands/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/userCommands"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /userCommands existing resource", async () => {
