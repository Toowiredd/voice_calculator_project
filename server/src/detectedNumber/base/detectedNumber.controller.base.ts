/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import * as common from "@nestjs/common";
import * as swagger from "@nestjs/swagger";
import { isRecordNotFoundError } from "../../prisma.util";
import * as errors from "../../errors";
import { Request } from "express";
import { plainToClass } from "class-transformer";
import { ApiNestedQuery } from "../../decorators/api-nested-query.decorator";
import { DetectedNumberService } from "../detectedNumber.service";
import { DetectedNumberCreateInput } from "./DetectedNumberCreateInput";
import { DetectedNumber } from "./DetectedNumber";
import { DetectedNumberFindManyArgs } from "./DetectedNumberFindManyArgs";
import { DetectedNumberWhereUniqueInput } from "./DetectedNumberWhereUniqueInput";
import { DetectedNumberUpdateInput } from "./DetectedNumberUpdateInput";

export class DetectedNumberControllerBase {
  constructor(protected readonly service: DetectedNumberService) {}
  @common.Post()
  @swagger.ApiCreatedResponse({ type: DetectedNumber })
  async createDetectedNumber(
    @common.Body() data: DetectedNumberCreateInput
  ): Promise<DetectedNumber> {
    return await this.service.createDetectedNumber({
      data: data,
      select: {
        createdAt: true,
        id: true,
        updatedAt: true,
        value: true,
      },
    });
  }

  @common.Get()
  @swagger.ApiOkResponse({ type: [DetectedNumber] })
  @ApiNestedQuery(DetectedNumberFindManyArgs)
  async detectedNumbers(
    @common.Req() request: Request
  ): Promise<DetectedNumber[]> {
    const args = plainToClass(DetectedNumberFindManyArgs, request.query);
    return this.service.detectedNumbers({
      ...args,
      select: {
        createdAt: true,
        id: true,
        updatedAt: true,
        value: true,
      },
    });
  }

  @common.Get("/:id")
  @swagger.ApiOkResponse({ type: DetectedNumber })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  async detectedNumber(
    @common.Param() params: DetectedNumberWhereUniqueInput
  ): Promise<DetectedNumber | null> {
    const result = await this.service.detectedNumber({
      where: params,
      select: {
        createdAt: true,
        id: true,
        updatedAt: true,
        value: true,
      },
    });
    if (result === null) {
      throw new errors.NotFoundException(
        `No resource was found for ${JSON.stringify(params)}`
      );
    }
    return result;
  }

  @common.Patch("/:id")
  @swagger.ApiOkResponse({ type: DetectedNumber })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  async updateDetectedNumber(
    @common.Param() params: DetectedNumberWhereUniqueInput,
    @common.Body() data: DetectedNumberUpdateInput
  ): Promise<DetectedNumber | null> {
    try {
      return await this.service.updateDetectedNumber({
        where: params,
        data: data,
        select: {
          createdAt: true,
          id: true,
          updatedAt: true,
          value: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }

  @common.Delete("/:id")
  @swagger.ApiOkResponse({ type: DetectedNumber })
  @swagger.ApiNotFoundResponse({ type: errors.NotFoundException })
  async deleteDetectedNumber(
    @common.Param() params: DetectedNumberWhereUniqueInput
  ): Promise<DetectedNumber | null> {
    try {
      return await this.service.deleteDetectedNumber({
        where: params,
        select: {
          createdAt: true,
          id: true,
          updatedAt: true,
          value: true,
        },
      });
    } catch (error) {
      if (isRecordNotFoundError(error)) {
        throw new errors.NotFoundException(
          `No resource was found for ${JSON.stringify(params)}`
        );
      }
      throw error;
    }
  }
}
